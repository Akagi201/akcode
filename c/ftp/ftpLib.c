 ...
 *  @brief  File Transfer Protocol (FTP) library,Copyright 1984 - 2002 Wind River Systems, Inc.
...
#include "Davinci/davinci.h"
#include "Davinci/ftpLib.h"//替换成basefun.h
#include "base_fun.h"
#include "dev_debug.h"
...
int ftplDebug = FTPL_DEBUG_ERRORS;
...
#define FTP_DEBUG(level, arg...) dev_debug(level, MOD_SYSFUN_FTP, ##arg)
...
LOCAL STATUS ftpPasvReplyParse (char *, UINT32 *, UINT32 *, UINT32 *, UINT32 *, UINT32 *, UINT32 *);
LOCAL BOOL ftpTransientFatal (UINT32 reply);
LOCAL char pasvReplyString[PASV_REPLY_STRING_LENGTH];


static int ftpReplyGetEnhanced(int ctrlSock, BOOL expecteof, char *replyString, int  stringLengthMax);
...
STATUS(*_func_ftpTransientFatal)(UINT32 relay)    = ftpTransientFatal;

/** @fn	INT32 set_nonblock(INT32 sock_fd, BOOL b_set)
 *  @brief	设置socket fd为阻塞模式或者非阻塞模式
 *  @param[in]  sock_fd 已经连接成功的连接fd。范围:大于0
 *  @param[in]  b_set 是否设置为非阻塞模式。范围:非0，是；0，否
 *  @param[out] 无
 *  @return 成功返回0；失败返回-1
 */
static INT32 set_nonblock(INT32 sock_fd, BOOL b_set)
{
	INT32 flags = 0;
	...
	if (-1 == flags)
	{
		FTP_DEBUG(SYS_ERROR, "get sockfd flag error, errno=%d\n",  errno);
		return -1;
	}
	...
	if (b_set)
	{
		...
		{
			FTP_DEBUG(SYS_ERROR, "set sockfd nonblock error, errno=%d\n",  errno);
			return -1;
		}
	}
	else
	{
		if (fcntl(sock_fd, F_SETFL, flags & (~O_NONBLOCK)) == -1)
		{
			FTP_DEBUG(SYS_ERROR, "set sockfd block error, errno=%d\n",  errno);
			return -1;
		}
	}
...
/** @fn		  int ftpCommand(int ctrlSock, char *fmt, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6)
 *  @brief		send an FTP command and get the reply
 ...
 * 				 .CE
 *  @param[in]  ctrlSock: fd of control connection socket, control socket的文件描述符
 *  @param[in]  fmt: format string of command to send, 待传递的ftp命令
 *  @param[in]  arg1: first of six args to format string, ftp命令的参数
 *  @param[in]  int arg2
 *  @param[in]  int arg3
 *  @param[in]  int arg4
 *  @param[in]  int arg5
...
/** @fn		  int ftpCommandEnhanced(int ctrlSock, char *fmt, int arg1, int arg2, int arg3, int arg4, 
 *				int arg5, int arg6, char *replyString, int replyStringLength)
 *  @brief	send an FTP command and get the complete RFC reply code
 ...
 * 			 .CE
 *  @param[in]	int ctrlSock: fd of control connection socket
 *  @param[in]  char *fmt: format string of command to send
 *  @param[in]  int arg1: first of six args to format string
 *  @param[in]  int arg2: 
 *  @param[in]  int arg3: 
 *  @param[in]  int arg4: 
 *  @param[in]  int arg5: 
 *  @param[in]  int arg6: 
 *  @param[in]  char *replyString: storage for the last line of the server response or NULL
 *  @param[in]  int replyStringLength: Maximum character length of the replyString
 *  @param[out] 无
 *  @return	  出错: ERROR   成功: ftpReplyGetEnhanced(), ftpReplyGet()
 */
		...
		int arg5, int arg6, char *replyString, int replyStringLength)
{
	...
	if (writen(ctrlSock, buffer, len) < len)
	{
		FTP_DEBUG(SYS_ERROR, "ftpCommandEnhanced: error during write to control socket - errno:%d\n",  errno);
		return(ERROR);
	}
...
/** @fn		STATUS ftpXfer(char *host, unsigned short ftp_port, char *user, char *passwd, char *acct, char *cmd, 
  *				char *dirname, char *filename, int *pCtrlSock, int *pDataSock)  
 *  @brief	initiate a transfer via FTP
 ...
 * 			 .CS
 *  @param[in] 
 *  @param[in]  host: ftp服务器的IP地址，点分十进制，格式类似于127.0.0.1 或 域名地址
 *  @param[in]  user: 访问ftp服务器的用户名
 *  @param[in]  passwd: 访问ftp服务器的密码
 *  @param[in]  acct: account for host login, 一般是一个邮件地址,非必须, 直接写为空字符串就可以了
 *  @param[in]  cmd: 发送给FTP服务器的命令, 例如读取命令为"RETR"
 *  @param[in]  dirname: directory to 'cd' to before sending command, 待访问文件所在的目录
 *  @param[in]  filename: filename to send with command, 待访问文件的文件名
 *  @param[in]  pCtrlSock: where to return control socket fd, 存储返回的control socket文件描述符，也就是常说的fd
 *  @param[in]  pDataSock: where to return data socket fd, (NULL == don't open data connection)存储返回的待访问文件的文件描述符
 *  @param[out] 无
		...
		char *dirname, char *filename, int *pCtrlSock, int *pDataSock)
{
	...
	FTP_DEBUG(SYS_ERROR, "ftpXfer: host:%s user:%s passwd:%s acct:%s cmd:%s dir:%s\n",  \
			host,user,passwd,acct,cmd,dirname);

	if (((ctrlSock = ftpHookup (host, ftp_port)) == ERROR)    ||
					...
					(int)dirname,0,0,0,0,0) != FTP_COMPLETE)))
	{
		...
		FTP_DEBUG(SYS_ERROR, "ftpXfer: Detected an error during command establishment errno:0x%08x",  \
				errno);
		close (ctrlSock);
		return (ERROR);
	}
	...
	if (pDataSock == NULL)
	{
		...
		{
			/* FTP command error. */
			FTP_DEBUG(SYS_ERROR, "ftpXfer: error during command.  errno:0x%08x", errno);
	...
	do 
	{
		...
		{
			FTP_DEBUG(DEBUG_NOTICE, "ftpXfer: notice - mode succeeded.\n ");
			dataSockPassive = TRUE; /* We do not need to listen() on the socket */
		}
		else
		{
			FTP_DEBUG(SYS_ERROR, "ftpXfer: notice - PASV mode failed. Now trying older PORT connect method\n");

			if ((dataSock = ftpDataConnInit (ctrlSock)) == ERROR)
			{
				FTP_DEBUG(SYS_ERROR, "ftpXfer:  ftpDataConnInit - error during trying of another port. errno:%d\n", errno);
				close (ctrlSock);
				return (ERROR);
			} 
			else
		...
		if ((cmdResult/100) != FTP_PRELIM)
		{
			...
			{
				FTP_DEBUG(SYS_ERROR, "ftpXfer:  response 0x%08x - aborting transfer.\n", cmdResult);
				close (ctrlSock);
				return (ERROR);
			}
			...
			{
				FTP_DEBUG(SYS_ERROR, "ftpXfer:  calling user-supplied applette to see if %d FTP_TRANSIENT is fatal for this command.\n", cmdResult);
				if ((* _func_ftpTransientFatal) (cmdResult) == TRUE)
				{
					FTP_DEBUG(SYS_ERROR, "ftpXfer:  user-supplied applette says %d FTP_TRANSIENT  ** IS ** fatal for this command.\n", cmdResult);
					close (ctrlSock);
					errno = S_ftpLib_FATAL_TRANSIENT_RESPONSE;
					return (ERROR);
				}

				FTP_DEBUG(SYS_ERROR, "ftpXfer:  user-supplied applette says %d is  ** NOT ** fatal for this command.\n", cmdResult);
			}
			...
			if ((ftpReply = (cmdResult/100)) == FTP_TRANSIENT)
			{
				...
				{
					++retryCount;
					FTP_DEBUG(SYS_ERROR, "ftpXfer: warning - reply was %d(decimal) - FTP_PRELIM - #%d attempt in %d ticks.\n", cmdResult, retryCount, ftplTransientRetryInterval);
					if (ftplTransientRetryInterval)
						usleep(10*1000*ftplTransientRetryInterval);
				...
				else
				{
					...
					FTP_DEBUG(SYS_ERROR, "ftpXfer: error - reply was %d(decimal) - FTP_PRELIM - attempt limit (%d) exceeded.\n", cmdResult, ftplTransientMaxRetryCount);
					close (ctrlSock);
				...
				FTP_DEBUG(SYS_ERROR, "ftpXfer: error - ftpCommand != FTP_PRELIM.  errno:%d\n", errno);
				close (ctrlSock);
				return (ERROR);
			}
		}
		...
		if ( dataSockPassive == FALSE)
		{
			...
			FTP_DEBUG(SYS_ERROR, "ftpXfer: notice - cmdResult:%d dataSock:%d ctrlSock:%d  errno:%d\n", cmdResult, dataSock, ctrlSock, errno);
			...
			if (select (width, &readFds, NULL, NULL, &timeout) == ERROR)
			{
				FTP_DEBUG(SYS_ERROR, "ftpXfer: error - select()==ERROR. errno:%d\n", errno);
				close (dataSock);
				close (ctrlSock);
				return (ERROR);
			}
			...
			if (FD_ISSET (ctrlSock, &readFds) && ! FD_ISSET (dataSock, &readFds))
			{
				...
				FTP_DEBUG(SYS_ERROR, "ftpXfer: warning - control socket ready but data socket not ready\n");
				...
				FTP_DEBUG(SYS_ERROR, "ftpXfer:  error - sending QUIT command to host. errno:%d\n", errno);
	...
	if (!dataSockPassive)
	{
		...
		{
			FTP_DEBUG(SYS_ERROR, "ftpXfer: error - ftpDataConnGet()==ERROR. errno:%d\n", errno);
			close (ctrlSock);
			return (ERROR);
		}
	}
...
/** @fn		 int ftpReplyGet(int ctrlSock, BOOL expecteof) 
 *  @brief	get an FTP command reply  
 ...
 *			  4 = FTP_TRANSIENT (transient negative completion)
 *			  5 = FTP_ERROR (permanent negative completion)
 *  @param[in]  int ctrlSock        control socket fd of FTP connection 
 *  @param[in]  BOOL expecteof      TRUE = EOF expected, FALSE = EOF is error 
 *  @param[out] 
 *  @return	  1 = FTP_PRELIM (positive preliminary)
 *			  2 = FTP_COMPLETE (positive completion)
 *			  3 = FTP_CONTINUE (positive intermediate)
 *			  4 = FTP_TRANSIENT (transient negative completion)
 *			  5 = FTP_ERROR (permanent negative completion)
...
/** @fn		  int ftpReplyGetEnhanced(int ctrlSock, BOOL expecteof, char *replyString, int  stringLengthMax)
 *  @brief	get an FTP command reply
 ...
 *  @param[in]  int ctrlSock,      control socket fd of FTP connection
 *  @param[in]  BOOL expecteof,    TRUE = EOF expected, FALSE = EOF is error
 *  @param[in]  char *replyString, Location to store text of reply, or NULL 
 *  @param[in]  int  stringLengthMax   Maximum length of reply (not including NULL)
 *  @param[out] 
 *  @return	  The complete FTP response code (see RFC #959)
 *			  ERROR if there is a read/write error or an unexpected EOF.
 */
int ftpReplyGetEnhanced(int ctrlSock, BOOL expecteof, char *replyString, int  stringLengthMax)
{
	char c;
	FAST int codeType;
	FAST int code;
	...
	do
	{
		...
		while (!(eof = (readn (ctrlSock, &c, 1) == 0)) && (c != '\n'))
		{
	...
	if (eof & !expecteof)
	{
		FTP_DEBUG(SYS_ERROR, "ftpReplyGet:  error - experienced eof - errno:%d\n", errno);
		return (ERROR);
	}
	else
...
/** @fn		 int ftpHookup(char *host) 
 *  @brief	get a control connection to the FTP server on a specified host
 ...
 *  @param[in]  char *host     server host name or inet address
 *  @param[out] 无
 *  @return	 The file descriptor of the control socket, or ERROR if the Internet
...
int ftpHookup(char *host, unsigned short ftp_port)
{
	...
	struct timeval timeout;
	struct hostent* pstru_host = NULL;
	struct in_addr* p_addr = NULL;
	
	// 通过获取主机网络相关信息
	pstru_host = gethostbyname(host);
	if (pstru_host != NULL)
	{
		if (pstru_host->h_addr != NULL)
		{
			p_addr = (struct in_addr*)pstru_host->h_addr;
			inetAddr = p_addr ->s_addr;
		}
	}
	
	if (((inetAddr = (int) inet_addr (host)) == ERROR) && (0 == inetAddr ))
	{
		return (ERROR);
	}
	...
	if (ctrlSock < 0)
	{
		FTP_DEBUG(SYS_ERROR, "ftpHookup: error - failure to get socket. errno:%d\n", errno);
		return (ERROR);
	}
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpHookup: error - failure to bind socket. errno:%d\n", errno);
		close (ctrlSock);
		return (ERROR);
	}
	...
	timeout.tv_sec = 5;
	timeout.tv_usec = 0;
	if (connect_with_timeout (ctrlSock, (struct sockaddr *)&ctrlAddr, sizeof (ctrlAddr), &timeout) < 0)
	{ 
		FTP_DEBUG(SYS_ERROR, "ftpHookup: error - failure to connect socket. errno:%d\n", errno);
		close (ctrlSock);
		return (ERROR);
	}

	set_nonblock(ctrlSock, TRUE);
...
/** @fn		  STATUS ftpLogin(FAST int ctrlSock, char *user, char *passwd, char *account)
 *  @brief	log in to a remote FTP server 
 ...
 *  @param[in]  FAST int ctrlSock,  fd of login control socket
 *  @param[in]  char *user,         user name for host login
 *  @param[in]  char *passwd,       password for host login
 *  @param[in]  char *account       account for host login
 *  @param[out] 无
 *  @return	  OK, or ERROR if the routine is unable to log in.
 */
STATUS ftpLogin(FAST int ctrlSock, char *user, char *passwd, char *account)
{
	...
	if (n != FTP_COMPLETE)
	{
		FTP_DEBUG(SYS_ERROR, "ftpLogin: error - failure to get complete login. errno:%d\n", errno);
		return (ERROR);
	}
...
/** @fn		  int ftpDataConnInitPassiveMode(int ctrlSock)
 *  @brief	initialize an FTP data connection using PASV mode 
 ...
 *  @param[in]  int ctrlSock : fd of associated control socket
 *  @param[out] 无
 *  @return	  The file descriptor of the data socket created, or ERROR.
 */
int ftpDataConnInitPassiveMode(int ctrlSock)
{
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInitPassiveMode:  getsockname() failure. errno:%d\n", errno);
		return (ERROR);
	}
	...
	if (result == FTP_PASSIVE_REPLY)  /* The remote FTP server supports PASSIVE mode */
	{
		...
		if (dataSock < 0)
		{
			FTP_DEBUG(SYS_ERROR, "ftpDataConnInitPassiveMode: error - failure to get socket. errno:%d\n", errno);
			return (ERROR);
		}

		bzero ((char *) &dataAddr, sizeof (struct sockaddr));
		...
		{
			FTP_DEBUG(SYS_ERROR, "ftpDataConnInitPassiveMode:  getpeername() failure. errno:%d\n", errno);
			close (dataSock);
			return (ERROR);
		}
		...
		timeout.tv_sec = 5;
		timeout.tv_usec = 0;
		if (connect_with_timeout (dataSock, (struct sockaddr *)&dataAddr, sizeof (dataAddr), &timeout) < 0)
		{ 
			FTP_DEBUG(SYS_ERROR, "ftpDataConnInitPassiveMode: failure to connect. sock:%d sockMsb:%d sockLsb:%d errno:0x%08x\n", \
					 hostDataPort, portMsb, portLsb, errno);
			close (dataSock);
			return (ERROR);
		}
		...
		{
			FTP_DEBUG(DEBUG_NOTICE, "ftpDataConnInitPassiveMode: passive ftp connect established to host:%#x port:%d sock:%d\n", \
					dataAddr.sin_addr.s_addr, hostDataPort, dataSock);
			set_nonblock(dataSock, TRUE);
			return (dataSock);
		}
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInitPassiveMode: host failed to respond correctly to PASV command. errno:%d\n", errno);

		return (ERROR);
	}
}

/** @fn		  int ftpDataConnInit(int ctrlSock)
 *  @brief	initialize an FTP data connection using PORT mode 
 *			 This routine sets up the client side of a data connection for the
...
int ftpDataConnInit(int ctrlSock)
{
	...
	if (getsockname (ctrlSock, (struct sockaddr *)&ctrlAddr, &len) < 0)
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit:  getsockname() failure. errno:%d\n", errno);
		return (ERROR);
	}
	...
	if (dataSock < 0)
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit:  socket() failure. errno:%d\n", errno);
		return (ERROR);
	}
	set_nonblock(dataSock, TRUE);
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit:  bind() failure. errno:%d\n", errno);
		close (dataSock);
		return (ERROR);
	}
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit:  listen() failure. errno:%d\n", errno);
		close (dataSock);
		return (ERROR);
	}
...
#define UCP(n)     (((int)(((char *)&dataAddr.sin_port)[n])) & 0xff)
	...
	if (result != FTP_ERROR)
	{
		...
		{
			FTP_DEBUG(SYS_ERROR, "ftpDataConnInit:warning got FTP_PRELIM\n");
		}

		if (result != FTP_COMPLETE && result != FTP_PRELIM)
		{
			FTP_DEBUG(SYS_ERROR, "ftpDataConnInit: error - reply was %d(decimal) - not FTP_COMPLETE or FTP_PRELIM.\n", result);
			close (dataSock);
			return (ERROR);
		}
		else
	...
	if (dataSock < 0)
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit: error - 2nd try to get port number failed on socket(). errno:%d\n", errno);
		return (ERROR);
	}
	set_nonblock(dataSock, TRUE);
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit: error -  bind failed on 2nd attempt  errno :%d\n", errno);
		close (dataSock);
		return (ERROR);
	}
	...
	if (listen (dataSock, 1) < 0)
	{
		FTP_DEBUG(SYS_ERROR, "ftpDataConnInit: error -  listen failed on 2nd attempt  errno :%d\n", errno);
		close (dataSock);
		return (ERROR);
	}
...
/** @fn		  int ftpDataConnGet(int dataSock)
 *  @brief	get a completed FTP data connection  
 ...
 *  @param[in]  int dataSock       fd of data socket on which to await connection 
 *  @param[out] 无
 *  @return	The file descriptor of the new data socket, or ERROR if the connection failed
 */
...
/** @fn		  STATUS ftpLs(char * dirName)
 *  @brief		list directory contents via FTP
 ...
 *  @param[in]  char * dirName : name of directory to list 
 *  @param[out] 无
 *  @return	 OK, or ERROR if could not open directory.
 */
STATUS ftpLs(char * dirName)
{
#if 0
	DEV_HDR * pDevHdr;
...
/** @fn		  void ftpLibDebugOptionsSet(UINT32 debugLevel)
 *  @brief	set the debug level of the ftp library routines  
 ...
 *			 .CE
 *  @param[in]  UINT32 debugLevel
 *  @param[out] ftplDebug: 	调试等级		
 *  @return	  无
 */
...
/** @fn	  STATUS ftpTransientConfigSet(UINT32 maxRetryCount, UINT32 retryInterval)
 *  @brief	set parameters for host FTP_TRANSIENT responses  
 ...
 *  @param[in]  UINT32 maxRetryCount,The maximum number of attempts to retry 
 *  @param[in]  UINT32 maxRetryCount,The maximum number of attempts to retry 
 *  @param[out] 无
 *  @return	  OK
 */
STATUS ftpTransientConfigSet(UINT32 maxRetryCount, UINT32 retryInterval)
{
...
/** @fn		 STATUS ftpTransientConfigGet(UINT32 *maxRetryCount, UINT32 *retryInterval) 
 *  @brief	get parameters for host FTP_TRANSIENT responses 
 ...
 *  @param[in]  UINT32 *maxRetryCount  The maximum number of attempts to retry
 *  @param[in]  UINT32 *retryInterval  time (in system clock ticks) between retries
 *  @param[out] 无
 *  @return	  OK
 */
STATUS ftpTransientConfigGet(UINT32 *maxRetryCount, UINT32 *retryInterval)
{
...
/** @fn		LOCAL BOOL ftpTransientFatal (UINT32 reply)  
 *  @brief	applette to terminate FTP transient host responses	 
 ...
 * 			the retry sequence.
 *  @param[in]  UINT32 reply   Three digit code defined in RFC #959 
 *  @param[out] 
 *  @return	  terminate retry attempts:TRUE; continue retry attempts:FALSE
 */
LOCAL BOOL ftpTransientFatal (UINT32 reply)
{
	switch (reply)
...
/** @fn		  STATUS ftpTransientFatalInstall(FUNCPTR pApplette)
 *  @brief	set applette to stop FTP transient host responses  
 ...
 *  @param[in]  FUNCPTR pApplette: function that returns TRUE or FALSE 
 *  @param[out] 无
 *  @return	  成功: OK  出错: ERROR
 */
STATUS ftpTransientFatalInstall(FUNCPTR pApplette)
{
	...
	_func_ftpTransientFatal = (STATUS (*)(UINT32))pApplette;

	return (OK); /* attempt to retry the last command */
}

/** @fn		  LOCAL STATUS ftpPasvReplyParse (char *responseString, UINT32 *argument1, UINT32 *argument2,
 *				UINT32 *argument3, UINT32 *argument4, UINT32 *argument5, UINT32 *argument6)
 *  @brief	Parse the reply of a PASV command    
 ...
 *  @param[in]    char *responseString, NULL terminated string 
 *  @param[in]    UINT32 *argument1,     First argument 
 *  @param[in]    UINT32 *argument2,     Second argument 
 *  @param[in]    UINT32 *argument3,     Third argument 
 *  @param[in]    UINT32 *argument4,     Fourth argument 
 *  @param[in]    UINT32 *argument5,     Fifth argument 
 *  @param[in]    UINT32 *argument6      Six argument 
 *  @param[out] 无
 *  @return	  成功: OK  出错: ERROR
 */
LOCAL STATUS ftpPasvReplyParse (char *responseString, UINT32 *argument1, UINT32 *argument2,
		UINT32 *argument3, UINT32 *argument4, UINT32 *argument5, UINT32 *argument6)
{
	char *index;
	UINT32 tmpArg1;
	...
	{
		FTP_DEBUG(SYS_ERROR, "ftpPasvReplyParse: error - '227' not part of PASV response\n");
		return (ERROR);
	}
	...
	if (index == NULL)
	{
		FTP_DEBUG(SYS_ERROR, "ftpPasvReplyParse: error - '(' not part of PASV response\n");
		return (ERROR);
	}
...
/**@brief	发送断点续传命令，即后面发送的数据都将追加到文件p_filename	  
 * @param[in]  ctl_sockfd 命令通道的连接fd
 * @param[in]  p_filename 要续传的文件名
 * @param[out] 无
 *  @return	  出错: ERROR
 *  			1 = FTP_PRELIM (positive preliminary)
 *  			2 = FTP_COMPLETE (positive completion)
...
{
	return (ftpCommand(ctl_sockfd, "APPE %s", (int)p_filename, 0, 0, 0, 0, 0));
}

    ...
    loff_t *f_pos)
{
    ...
    while (!kfifo_len(dev->tekkamankfifo)) 
    { /* nothing to read */
...
out:
    ...
    if (printk_ratelimit())
    {
        printk(KERN_EMERG "\"%s\" did read %li bytes\n",current->comm, (long)retval);
    }
    return retval;
}
    ...
    loff_t *f_pos)
{
    ...
    if (printk_ratelimit())
    {
        printk(KERN_EMERG "\"%s\" did write %li bytes\n",current->comm, (long)count);
    }
    return retval;
}
    ...
    unsigned int cmd, unsigned long arg)
{
     ...
     */
    if (_IOC_TYPE(cmd) != SCULL_KFIFO_MAGIC)
    {
        return -ENOTTY;
    }
    if (_IOC_NR(cmd) > SCULL_KFIFO_MAXNR)
    {
        return -ENOTTY;
    }
...
void scull_kfifo_cleanup_module(void)
{
    ...
    {
        if (scull_kfifo_devices->tekkamankfifo) 	
        {
            kfifo_free(scull_kfifo_devices->tekkamankfifo);
...
#if 1
    if (tekkamanbuffer && keke)
    {
        kfree(tekkamanbuffer);
    }
#endif
    if (tekkaman) 
    {
...
int scull_kfifo_init_module(void)
{
    ...
    if (result < 0) 
    {
        printk(KERN_EMERG "scull: can't get major %d\n", scull_kfifo_major);
        return result;
    }

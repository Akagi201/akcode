...
/** @file globalfifo.c
  ...
  * @brief    A globalfifo driver as an example of char device drivers  
  * @brief    This example is to introduce poll,blocking and non-blocking access
  * 
  * @author   liuboyf1
  * @date     2012-9-25
  * @version  V1.0.0
  ...
  loff_t *ppos)
{
  ...
  down(&dev->sem); //获得信号量,此函数包含了进入睡眠的动作吗？
  add_wait_queue(&dev->r_wait, &wait); //进入读等待队列头,为何要在获得信号量后进入读等待队列头？

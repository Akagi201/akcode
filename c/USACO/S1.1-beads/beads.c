...
/*
 * Calculate number of beads gotten by breaking before character p and going in
 * direction dir, which is 1 for forward and -1 for backward.
 */
...
int nbreak(int p, int dir)
{
    ...
    for (n = 0; n < len; n++, i = mod(i + dir, len))
    {
        ...
        /*
         * If we've chosen a color and see a bead not white and not that color,
         * stop
         */
...
int main()
{
    ...
    /*
     * If the whole necklace can be gotten with a good break, we'll sometimes
     * coutn beads more than once. this can only happen when the whole necklace
     * can be taken, when beads that can be grabbed from the right of the break
     * can also be grabbed from the left.
     */
